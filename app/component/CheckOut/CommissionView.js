...
export default class CommissionView extends Component {
    constructor(props) {
        super(props);
    }

    render() {
        const { usableState, commission, unit, deducCommission, percent } = this.props;
        let imgSource = null;
        let disabled = false;
        let imageVIew = <View />;
        let textView = <OText
            style={styles.detailText}
            text={'TEXT_NO_COMMISSION'}
        />
        let isOpen = true;
        switch (usableState) {
        ...
        if (imgSource) {
            ...
            textView = <Text style={styles.detailText}>
                <OText
                    text={'TEXT_DEDUCT_CAN_USED_TO_APP_1'}
                />
                <Text style={styles.detailPrice}>{unit}{commission}
                </Text>
                <OText
                    text={'TEXT_DEDUCT_CAN_USED_TO_APP_2'}
                />
            </Text>
        }
        ...
        return <View style={styles.viewStyle}>
            <View>
                <TouchableOpacity
                    onPress={() => {
                        Alert.alert('', I18n('TEXT_DEDUCT_INTRO_COMMISSION', { value: percent * 100 }),
                            [{
                                text: I18n('CART_OK')
                            }])
                    }}
                ...
                >
                    <OText
                        ...
                        text={'TEXT_POINT_COMMISSION_DEDUCTION'}
                    />
                    <Image source={POINT} />
                </TouchableOpacity>
                {textView}
