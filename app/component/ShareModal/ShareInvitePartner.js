...
export default class ShareInvitePartner extends Component {
    constructor(props) {
        super(props);
        this.state = {
            showIndicator: false,
    ...
    componentWillMount() {
        NativeShare.asynSupportPlatform()//检测手机本身是否装有相关软件
            .then(result => {
                ...
                platform[2].enable = supFB;//facebook
                this.setState({
                ...
                () => {
                    InteractionManager.runAfterInteractions(() => {
                        try {
                            ...
                            const { nickname, store, headImgUrl } = shareData;
                            const { storeId } = store;
                            let link = AppSetting.BaseUrl + '/applypartner/' + storeId;
                            ...
                            if (//复制链接
                                ...
                                this.showToast(I18n("SHARE_MASK_COPY_SUCCESS"));//复制链接成功
                                this.setState({
                                    showIndicator: false
                                });
                                //this.closeModal();
                            ...
                            let title = I18n('PERSON.TEXT_WX_INVITE_TITLE',{name:nickname});
                            let desc = I18n('PERSON.TEXT_WX_INVITE_SUBTITLE');
                            console.log('111',title,desc)
